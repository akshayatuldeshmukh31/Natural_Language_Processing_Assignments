...
def generate_string_parse_tree(parse_table, start, tokenized_text, sentence_length):
	...
	while len(stack)!=0:
		...
		if current_node[1] != current_node[2]:
			output = output[:break_point] + "(" + current_node[0].strip().split("^")[0].strip() + " { {)" + output[break_point + 1:]
		...
		if current_node[1] == current_node[2]:
			output = output[:break_point] + "(" + current_node[0].strip().split("^")[0].strip() + " " + tokenized_text[current_node[1]] + ")" + output[break_point + 1:]
	...
	for l in range(length_tokenized_text):
		for i in range(length_tokenized_text):
			...
			if l==0:
				...
				# 	omissions = int(math.floor(len(parse_table[i][i+l])*0.1))
				...
				# 				# print "DELETION"
				# 				break
				# 	parse_table[i][i+l].sort(key=lambda x: x[1])
				# 	# print parse_table[i][i+l]
				# 	del parse_table[i][i+l][:0]
				# 	# print parse_table[i][i+l]
				# 	# print ""

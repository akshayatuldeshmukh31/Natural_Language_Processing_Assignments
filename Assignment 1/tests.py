...
until the fourth line"""
        ...
        g = """The limerick packs laughs anatomical
...
And the clean ones so seldom are comical."""
    ...
    def test_guess_syllables(self):
        s = []
        try: self.assertEqual(self.ld.guess_syllables("thou"), 2)
        except: s.append(1)
    ...
    def test_apos(self):
        s = []
        try: self.assertEqual(self.ld.apostrophe_tokenize("""And he replied, Oh! What've we done?"""), 1)
        except: s.append(1)
        # try: self.assertEqual(self.ld.guess_syllables("asdf"), 1)
        # except: s.append(2)
        # try: self.assertEqual(self.ld.guess_syllables("letter"), 2)
        ...
        print '\nNumber of failed apos syllables tests:', str(len(s))
        if len(s)!=0: print 'Failed apos syllables tests:', ','.join([str(x) for x in s])
